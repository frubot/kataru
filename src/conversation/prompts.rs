...
### "ナレーション"に関して
  ...
  ナレーションにおける制約:
   - 感情や動作、行動、状況に関連するものはナレーションとして叙述的に説明してください。文体はキャラクター設定に影響されません。
   - *説明文* のように * で囲っで説明します。
...
### ナレーションの出力例
  *放課後の教室。あなたの発言を聞いて太郎は興奮した様子で話す*そうなんだよ！マジやばくね？*目を輝かせてそのまま勢いで机に手を付き、顔を近づける。そして真っすぐな目で見つめる。*
