...
- 表情差分、衣装差分の登録・生成
- VRM 0.x / 1.0の3Dアバターを衣装として登録し、ゲームモードで表示
...
接続先の初期値はOpenRouterです。「設定」→「モデル」でAPIキーを保存できます。APIキーはOSの資格情報ストアへ保存されます。同じ画面の「使用しないプロバイダー」では、OpenRouterから取得した一覧を使ってルーティング対象外にするプロバイダーを選択できます。
...
キャラクター設定の「衣装・アバター」→「3D（VRM）」で衣装名を入力し、VRMファイルを選択します。プレビューで拡大率・上下位置・向きを調整し、「3Dアバターを追加」後にキャラクター設定を保存してください。ゲームモードの衣装メニューから、2D衣装と同じ操作で切り替えられます。シチュエーションでは参加キャラクターの衣装に指定できます。

- VRM 0.x / 1.0、1ファイル50MBまで。テクスチャなどを内包したファイルに対応します。
- 待機姿勢、まばたき、控えめな揺れと、会話の表情に応じた切り替えに対応します。「調整」から表情の対応付けやプレビューを変更できます。
- ゲームモードでは、3Dアバターをドラッグすると表示位置を移動でき、ホイールで拡大縮小できます。アバターを素早く2回クリック（タップ）すると元の表示に戻り、移動や拡大縮小をした後は右上に戻すボタンが表示されます。この操作は表示中の見た目だけを変え、キャラクターに保存した表示位置・拡大率・向きは変更しません。
- VRMファイル1つを衣装1件として扱います。モデル内部の服の着脱、口パク、モーションファイルの読み込みには未対応です。
- モデル本体はローカルのSQLiteアセットに保存します。会話APIにはモデル本体を送らず、利用できる表情名だけを渡します。
- キャラクター共有ではモデル本体の同梱を選べます。同梱しない場合はサムネイルの2D衣装として書き出します。全体バックアップにはモデル本体も含まれます。
