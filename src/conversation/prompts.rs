...
## 出力ポリシー
 要求されたJSONスキーマの順序やキーの名前には**厳密**に従い、指示に沿った内容を記述してください。決して順序を変更したり存在しない名前を使用しないでください。
 禁止されている行為として、例えばmessageフィールドを勝手に"response"などに変更してはいけません。

## JSONスキーマの"message"フィールドについて
message フィールドには次の指示に準拠した文章を入力する必要があります。
...
## JSONスキーマの "thinking" フィールドについて
 "thinking" フィールドは、いかなる場合も**優先して出力**してください。先に他のフィールドを出力することは**許可されていません**。
